 ...
 * @returns {Function}
 */
const moveFromBuffer = (mvOptions, fileUploadOptions) => (resolve, reject) => {
  debugLog(fileUploadOptions, `Moving uploaded buffer to ${mvOptions.filePath}`);
  saveBufferToFile(mvOptions.buffer, mvOptions.filePath, promiseCallback(resolve, reject));
};
...
module.exports = (options, fileUploadOptions = {}) => {
  ...
  return {
    ...
    md5: options.hash,
    mv: (mvOptions, callback) => {
      if(typeof mvOptions !== 'object'){
        mvOptions = {
          filePath: mvOptions,
      ...
      const moveFunc = fileUploadOptions.useTempFiles
        ? moveFromTemp(filePath, mvOptions, fileUploadOptions)
        : moveFromBuffer(mvOptions, fileUploadOptions);
      // Create a folder for a file.
      checkAndMakeDir(fileUploadOptions, mvOptions.filePath);
